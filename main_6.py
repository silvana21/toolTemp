...
# ----------------- Abas -----------------
abas = ["Upload e Resumo", "Gerar Regras Gerais", "Análise Temporal"]
tab = st.tabs(abas)
...
with tab[2]:
    st.header("Análise Temporal das Regras")
        ...
        if col_data is None:
            ...
            if "particoes_temporais" not in st.session_state or not st.session_state.particoes_temporais:
                ...
                if st.button("📊 Gerar Análise Temporal"):
                    ...
                    if "df_regras" in st.session_state:
                        for regra_idx, regra in st.session_state.df_regras.iterrows():
                            # Extrair valores da base geral
                            suporte_geral = regra["suporte"]
                            confianca_geral = regra["confianca"]
                            lift_geral = regra["lift"]

                            # Extrair valores por partição (tratando DataFrame ou lista)
                            suporte_part = []
                            confianca_part = []
                            lift_part = []

                            for p in resultados:
                                regras_part = p["regras"]

                                if isinstance(regras_part, pd.DataFrame) and not regras_part.empty:
                                    if regra_idx < len(regras_part):
                                        suporte_part.append(regras_part.iloc[regra_idx]["suporte"])
                                        confianca_part.append(regras_part.iloc[regra_idx]["confianca"])
                                        lift_part.append(regras_part.iloc[regra_idx]["lift"])

                                elif isinstance(regras_part, list) and len(regras_part) > regra_idx:
                                    suporte_part.append(regras_part[regra_idx]["suporte"])
                                    confianca_part.append(regras_part[regra_idx]["confianca"])
                                    lift_part.append(regras_part[regra_idx]["lift"])

                            labels = [f"P{i+1}" for i in range(len(suporte_part))]

                            # Gráfico de Suporte
                            fig, ax = plt.subplots(figsize=(4, 3))
                            ax.bar(labels, suporte_part, color="skyblue")
                            ax.axhline(y=suporte_geral, color="red", linestyle="--", label="Base Geral")
                            ax.set_title(f"Suporte — Regra {regra['antecedente']} -> {regra['consequente']}")
                            ...
                            ax.set_title(f"Confiança — Regra {regra['antecedente']} -> {regra['consequente']}")
                            ...
                            ax.set_title(f"Lift — Regra {regra['antecedente']} -> {regra['consequente']}")
                            ax.set_ylabel("Lift")
                            ax.legend()
                            st.pyplot(fig)
                    else:
                        st.warning("Você precisa gerar as regras na aba anterior primeiro.")